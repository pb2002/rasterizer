...
namespace Template
{
	class RenderTarget
	{
		...
		public RenderTarget( int screenWidth, int screenHeight, string lutTexture )
		{
			_width = screenWidth;
		...
		public void LoadLUT(string name)
		{
			...
			{
				// generate color LUT
				Bitmap bmp = new Bitmap(256, 16);

				for(int y = 0; y < 16; y++)
				{
					for (int x = 0; x < 256; x++)
					{
						// bit juggling magic \[T]/
						Color c = Color.FromArgb(255, (x << 4) & 0xFF, x & 0xF0, (y << 4) & 0xFF);
